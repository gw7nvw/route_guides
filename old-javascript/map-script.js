         ...
         for (x in features) {
            ...
            if (type == thistype) {
              if (style==null) {
                ...
                if (style == thisstyle) map_scratch_source.removeFeature(features[x]);
              }
            }
          }
      }
...
function map_enable_draw(type, style, loc_dest, x_dest, y_dest, move) {
	...
	map_draw.on('drawend', function (event) {
		if (move==true) map_clear_scratch_layer(type);
...
function map_get_current_extent(proj) {
   ...
   function map_WKTtoGPX(wktfield, gpxfield) {
     ...
     if (document.getElementById(wktfield).value) {
       var fea = wktp.readFeatures(document.getElementById(wktfield).value,{dataProjection: 'EPSG:4326', featureProjection:'EPSG:4326'});
       if (fea.length>0) {
         document.getElementById(gpxfield).value=gpxp.writeFeatures(fea[0],{dataProjection: 'EPSG:4326', featureProjection:'EPSG:4326'});
       }
     }
}


   function map_GPXtoWKT(gpx, wktfield) {
     var wktp = new ol.format.WKT;
     var gpxp = new ol.format.GPX;

     if (gpx) {
       var fea = gpxp.readFeatures(gpx,{dataProjection: 'EPSG:4326', featureProjection: 'EPSG:4326'});
       if (fea.length>0) {
        if(fea.length>1) {
         ...
         document.getElementById(wktfield).value=wktp.writeFeature(thefea,{dataProjection: 'EPSG:4326', featureProjection: 'EPSG:4326'});
       } else {
...
function map_trim_linestring_to_points(routewkt,startwkt,endwkt) {
     var wktp = new ol.format.WKT;
   
     var checkrtwkt=map_check_wkt_dimensions(routewkt);
     var route = wktp.readFeatures(checkrtwkt,{dataProjection: 'EPSG:4326', featureProjection:'EPSG:4326'});
     var start = wktp.readFeatures(startwkt,{dataProjection: 'EPSG:4326', featureProjection:'EPSG:4326'});
     var end = wktp.readFeatures(endwkt,{dataProjection: 'EPSG:4326', featureProjection:'EPSG:4326'});

     // get closest point to start
     var rtcoords=route[0].getGeometry().getCoordinates();
     var stcoords=start[0].getGeometry().getFirstCoordinate();
     var endcoords=end[0].getGeometry().getFirstCoordinate();
     var rtlength=rtcoords.length;
     var startindex;
     var startdist=null;
     var endindex;
     var enddist=null;
     for (var count=0; count<rtlength; count++) {
        xd=rtcoords[count][0]-stcoords[0];
        yd=rtcoords[count][1]-stcoords[1];
        tds=Math.sqrt(xd*xd+yd*yd);
        if ((tds<startdist) || (startdist==null) ) {
            startdist=tds;
            startindex=count;
        } 
     }    
     for (var count=startindex; count<rtlength; count++) {
        xd=rtcoords[count][0]-endcoords[0];
        yd=rtcoords[count][1]-endcoords[1];
        tds=Math.sqrt(xd*xd+yd*yd);
        if ((tds<enddist) || (enddist==null) ) {
            enddist=tds;
            endindex=count;
        } 
     }    

     var newrtcoords=rtcoords.slice(startindex,endindex);
     var newlinestr=new ol.geom.LineString(newrtcoords);
     var newfea=new ol.Feature({ geometry: newlinestr });
     var newrtwkt=wktp.writeFeature(newfea,{dataProjection: 'EPSG:4326', featureProjection: 'EPSG:4326'});
     return newrtwkt;
}

function map_check_wkt_dimensions(wkt) {
  var values=wkt.substr(wkt.indexOf('(')+1,wkt.indexOf(')')-1);
  var prefix=wkt.substr(0,wkt.indexOf('('));
  var firstvalues=values;
  if(values.indexOf(',')>0) firstvalues=values.substr(0,values.indexOf(','));
  var count = (firstvalues.match(/\ /g) || []).length;
  count++;
  switch(count) {
   case 2:
     dim_str='';
   ...
   case 4: 
     ...
     dim_str=' ZM';
   default:
     alert("Don't know how to handle a geometry with "+count+" dimensions")
     return wkt;
   }
  
   if(prefix.toUpperCase().indexOf('POINT')==0) {
      wkt="POINT"+dim_str+"("+values+")";
   }
   if(prefix.toUpperCase().indexOf('LINESTRING')==0) {
      wkt="LINESTRING"+dim_str+"("+values+")";

   }
   if(prefix.toUpperCase().indexOf('MULTILINESTRING')==0) {
      wkt="MULTILINESTRING"+dim_str+"("+values+")";
